...
namespace COServer.Game.MsgServer.AttackHandler
{
    public class Bomb
    {
        ...
        {
            if (Role.Core.Rate(2) && Attack.SpellID == (ushort)Role.Flags.SpellID.Poison)
            {
            ...
            if (CheckAttack.CanUseSpell.Verified(Attack, user, DBSpells, out ClientSpell, out DBSpell))
            {
                ...
                {
                    case (ushort)Role.Flags.SpellID.SpeedLightning:
                        {
                            if (!user.Player.ContainFlag(MsgUpdate.Flags.XPList))//لو مش معاه الxp مش هيضرب
                                return;
                            user.Player.RemoveFlag(MsgUpdate.Flags.XPList);//كده بقت مرة واحدة بس
                            var suc = false;
                    ...
                    case (ushort)Role.Flags.SpellID.FireCircle:
                        {
                            ...
                            foreach (Role.IMapObj targer in user.Player.View.Roles(Role.MapObjectType.Player))
                            {
                                ...
                                if (CheckAttack.CanAttackPlayer.Verified(user, attacked, DBSpell))
                                {
                                    //if (attacked.ContainFlag(MsgUpdate.Flags.Fly)) return;
                                    if (Calculate.Base.GetDistance(user.Player.X, user.Player.Y, attacked.X, attacked.Y) < DBSpell.Range)
                                    {
                                        MsgSpellAnimation.SpellObj AnimationObj;
                                        Calculate.Magic.OnPlayer(user.Player, attacked, DBSpell, out AnimationObj);
                    ...
                    case (ushort)Role.Flags.SpellID.Pravade:
                        {
                            MsgSpellAnimation MsgSpell = new MsgSpellAnimation(user.Player.UID
                               , 0, Attack.X, Attack.Y, ClientSpell.ID
                               , ClientSpell.Level, ClientSpell.UseSpellSoul);
                            ...
                            uint Experience = 0;
                            if (ClientSpell.Level == 0|| ClientSpell.Level == 1)
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.LocationEffect, true, "zf2-e263");
                            }
                            else if (ClientSpell.Level == 2)
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "zf2-e264");
                            }
                            else if (ClientSpell.Level == 3)
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "zf2-e265");
                            }
                            else if (ClientSpell.Level == 4)
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "zf2-e266");
                            }
                            else if (ClientSpell.Level == 5)
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "zf2-e267");
                            }
                            else
                            {
                                user.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "tj");
                            }
                            foreach (Role.IMapObj target in user.Player.View.Roles(Role.MapObjectType.Monster))
                            {
                                MsgMonster.MonsterRole attacked = target as MsgMonster.MonsterRole;
                            ...
                            foreach (Role.IMapObj targer in user.Player.View.Roles(Role.MapObjectType.Player))
                            {
                                ...
                                if (Calculate.Base.GetDistance(user.Player.X, user.Player.Y, attacked.X, attacked.Y) < DBSpell.Range)
                                {
                                    if (CheckAttack.CanAttackPlayer.Verified(user, attacked, DBSpell))
                                    {
                                        ...
                                        //AnimationObj.Damage = (uint)(AnimationObj.Damage * increasedamage / 15);
                                        ReceiveAttack.Player.Execute(AnimationObj, user, attacked);

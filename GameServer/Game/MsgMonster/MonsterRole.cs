...
namespace COServer.Game.MsgMonster
{
    public unsafe class MonsterRole : Role.IMapObj
    {
        ...
        // *** DragonBall Drop Effect ***
        private void SendEarthquakeEffect(Client.GameClient client, ServerSockets.Packet stream)
        {
            try
            {
                MsgFloorItem.MsgItemPacket effect = MsgFloorItem.MsgItemPacket.Create();
                effect.m_UID = 7; // EarthquakeAndNight
                effect.DropType = (MsgFloorItem.MsgDropID)13;
                ...
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    if (user.Player.Map == client.Player.Map)
                        user.Send(stream.ItemPacketCreate(effect));
                }

                Program.SendGlobalPackets.Enqueue(
                    new MsgMessage(
                        $"A Super Dragon Ball Dropped drom a monster killed by {client.Player.Name}!",
                        MsgMessage.MsgColor.red,
                        MsgMessage.ChatMode.Center
                    ).GetArray(stream)
                );
            }
            catch { }
        ...
        public void Dead(ServerSockets.Packet stream, Client.GameClient killer, uint aUID, Role.GameMap GameMap, bool CounterKill = false)
        {
            if (Alive)
            {
                ...
                RespawnStamp = Time32.Now.AddSeconds(1 + Family.RespawnTime);
                    ...
                    if (killer.Player.VipLevel >= 6)
                    {
                        ...
                        if (killer.DbKilled >= ProjectControl.VipDb_Drop)
                        {
                            ...
                            if (killer.Inventory.HaveSpace(ProjectControl.Max_DragonBall_Vip))
                            {
                                ...
                                SendEarthquakeEffect(killer, stream);
                            ...
                            for (byte i = 0; i < count; i++)
                            {
                                ...
                                if (ID != 0)
                                {
                                    if (killer.Map.AddGroundItem(ref xx, ref yy))
                                    {
                                        ...
                                        if (ID_Quality >= 8 || ID_Special || ID == 700001 || ID == 724515)
                                        {
                                        ...
                                        else if (DbItem != null && DbItem.Plus != null && DbItem.Plus.Length > 0)
                                        {
                                            DropItem(stream, killer.Player.UID, killer.Map, ID, xx, yy, MsgFloorItem.MsgItem.ItemType.Item, 0, ID_Special, ID_Quality, killer, DbItem);
                                        }
                                    }
        ...
        private void DropItem(ServerSockets.Packet stream, uint OwnerItem, Role.GameMap map, uint ItemID, ushort XX, ushort YY, MsgFloorItem.MsgItem.ItemType typ, uint amount, bool special, byte ID_Quality, Client.GameClient user = null, Database.ItemType.DBItem DBItem = null)
        {
            ...
            switch (ID_Quality)
            {
                case 9: DataItem.Color = Role.Flags.Color.Yellow; break; // Super
                case 8: DataItem.Color = Role.Flags.Color.Purple; break; // Elite
                case 7: DataItem.Color = Role.Flags.Color.LightBlue; break; // Unique
                case 6: DataItem.Color = Role.Flags.Color.Blue; break; // Refined
                default: DataItem.Color = Role.Flags.Color.White; break; // Normal
        ...
        {
            if (!Alive && State != MobStatus.Respawning)
            {
